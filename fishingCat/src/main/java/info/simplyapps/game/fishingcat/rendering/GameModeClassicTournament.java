...
public class GameModeClassicTournament extends FishRendererTemplate implements GameEngine {
	...
	public GameModeClassicTournament(Context context, Properties p) {
	    super(context, p);
	...
	private void createGame() {
		synchronized(StoreDataNew.getInstance()) {
			...
			if(GameHelper.hasBronze()) {
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_EASY][0]) { hasBronzeReward += 1; }
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_MEDIUM][0]) { hasBronzeReward += 1; }
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_HARD][0]) { hasBronzeReward += 1; }
			}
			if(GameHelper.hasSilver()) {
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_EASY][1]) { hasSilverReward += 1; }
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_MEDIUM][1]) { hasSilverReward += 1; }
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_HARD][1]) { hasSilverReward += 1; }
			}
			if(GameHelper.hasGold()) {
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_EASY][2]) { hasGoldReward += 1; }
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_MEDIUM][2]) { hasGoldReward += 1; }
				if(StoreDataNew.getInstance().gameData1.rewards[GameValues.GAMEMODE_HARD][2]) { hasGoldReward += 1; }
			}
		}
...
//			screenHeightStart = -1 * (fishHeight/3);
	 ...
	 * 
	 * @param mode
	 *            one of the GameState constants
	 */
	...
	private void clearSharks() {
		List<Animation> removals = new ArrayList<>();
		for (FishAnimation a : getSprites().aFish) {
			if (a.sharkFish) {
				removals.add(a);
			}
