...
import info.simplyapps.game.fishingcat.sprites.GameViewSpritesMode2;
import info.simplyapps.game.fishingcat.storage.DBDriver;
import info.simplyapps.game.fishingcat.storage.StoreDataNew;
import info.simplyapps.game.fishingcat.system.Game2;
	...
	private void createGame() {
		synchronized(StoreDataNew.getInstance()) {
			...
			if(GameHelper.hasBronze()) {
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_EASY][0]) { hasBronzeReward += 1; }
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_MEDIUM][0]) { hasBronzeReward += 1; }
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_HARD][0]) { hasBronzeReward += 1; }
			}
			if(GameHelper.hasSilver()) {
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_EASY][1]) { hasSilverReward += 1; }
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_MEDIUM][1]) { hasSilverReward += 1; }
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_HARD][1]) { hasSilverReward += 1; }
			}
			if(GameHelper.hasGold()) {
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_EASY][2]) { hasGoldReward += 1; }
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_MEDIUM][2]) { hasGoldReward += 1; }
				if(StoreDataNew.getInstance().gameData2.rewards[GameValues.GAMEMODE_HARD][2]) { hasGoldReward += 1; }
			}
		}
		mGame = new Game2();
