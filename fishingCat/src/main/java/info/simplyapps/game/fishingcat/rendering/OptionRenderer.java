...
public class OptionRenderer extends FishRendererTemplate implements IPermissionHandler {
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (containsClick(getSprites().rBtnVibration, event.getX(), event.getY())) {
                        ...
                        // disable if permission is denied
                        if (Boolean.valueOf(c.value).booleanValue() && !getScreen().getPermissionHelper().checkPermission(mContext, getScreen(), Manifest.permission.VIBRATE, true)) {
                            c.value = Boolean.toString(Boolean.FALSE);
                        }
                        if (DBDriver.getInstance().store(c)) {
                            SystemHelper.setConfiguration(c);
                        }
